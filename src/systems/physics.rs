...
impl<'a> System<'a> for Physics {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        //TODO: Apply forces: http://nphysics.org/rigid_body_simulations_with_contacts/#gravity-and-external-forces
        ...
        for (&entity, body) in self.bodies.iter() {
            if let &Body::RigidBody {body_handle, ..} = body {
                let position = positions.get_mut(entity).expect("Rigid body should have had a position");
                let velocity = velocities.get_mut(entity).expect("Rigid body should have had a velocity");

                let physics_body = self.world.rigid_body(body_handle).expect("Body handle did not map to a rigid body");
